#!/bin/bash
# Analyze with updated config excluding 3rdparty

CLANG_TIDY="/opt/homebrew/opt/llvm@19/bin/clang-tidy"
TEMP_OUTPUT="/tmp/clang_tidy_v3_$$.txt"

echo "Analyzing with refined configuration (excluding 3rdparty)..."
...
echo ""
echo "=== REFINED WARNING SUMMARY (excluding 3rdparty) ==="
echo ""
...
cp "$TEMP_OUTPUT" clang_tidy_warnings_v3.txt
echo "Full warnings saved to clang_tidy_warnings_v3.txt"
rm "$TEMP_OUTPUT"
