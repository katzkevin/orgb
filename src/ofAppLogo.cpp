...
void ofApp::applicationLogoHandler() {
    ...
    if (ofGetElapsedTimef() > showLogoForSeconds) {
        ...
        alpha = exponentialMap(ofGetElapsedTimef(), showLogoForSeconds - LOGO_FADE_OUT_FOR_SECONDS,
                               showLogoForSeconds, 255, 0, 0.75);
...
void ofApp::applicationLogoAndWebsiteHandler() {
    ...
    drawWebsite(static_cast<uint8_t>(alpha * instComponent), stof(getEnv("WEBSITE_OFFSET", "-5")) + overallOffset, "@kevinkatz.io");
    drawWebsite(static_cast<uint8_t>(alpha * siteComponent), stof(getEnv("WEBSITE_OFFSET", "-5")) + overallOffset, "www.kevinkatz.io");
