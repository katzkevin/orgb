    ...
    while (receiver.hasWaitingMessages()) {
        ...
        try {
            if (m.getAddress() == MIDI_KEYBOARD_OSC_ADDRESS) {
                ...
                json j = nlohmann::json::object();
                if (m.getNumArgs() <= 1) {
                    ofLogNotice("OSC") << "Got " << m.getNumArgs() << "-length " << MIDI_GUITAR_OSC_ADDRESS << " message. Skipping.";
                    continue;
                }
...
//                        (m.getArgAsTimetag(0) & 0xFFFFFFFF00000000) >> 32;                 // Stored in higher 32 bits
//                    uint64_t tagEpochTimeSecs = tagNineteenHundredTimeSecs - 2208988800L;  // Time between 1900 and 1970
//                    uint64_t tagTimeFrac = (m.getArgAsTimetag(0) & 0x00000000FFFFFFFF);    // Stored in higher 32 bits
//                    uint64_t tagTimeMicroseconds =
//                        (uint32_t)((double)tagTimeFrac * 1.0e6 /
//                                   (double)(1LL << 32));  // https://tickelton.gitlab.io/articles/ntp-timestamps/
//                    uint64_t tagEpochTimeTotalMicrosecond = tagEpochTimeSecs * 1000000 + tagTimeMicroseconds;
//                    uint64_t nowTotalMicrosecond = std::chrono::duration_cast<std::chrono::microseconds>(
//                                                       std::chrono::system_clock::now().time_since_epoch())
//                                                       .count();
//                    float totalLag = ((nowTotalMicrosecond - tagEpochTimeTotalMicrosecond) / 1.0e6);
//                    //                    ofLogVerbose("OSC") << tagNineteenHundredTimeSecs << " " << tagEpochTimeSecs
