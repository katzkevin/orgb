...
void PointWaves::drawUnit(ColorProvider & clr, KeyState & ks, DrawManager & dm, const Press & p, float alpha) {
    ...
    dm.shadeGlowCircle(ofVec2f(xCoord, centerY), std::min(halfZoneWidth, halfZoneHeight), pressColor,
                       modulatedIntensity, computedDampenRadius, blendMode, toneMap);
    // ofDrawPoint(horizontalCenterOfNotesZone - halfOfTheWidthOfNotesZone, ofGetHeight() / 2.0,
    // halfOfTheWidthOfNotesZone, 10);
}
