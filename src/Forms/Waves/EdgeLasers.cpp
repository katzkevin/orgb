...
void EdgeLasers::drawUnit(ColorProvider &clr, KeyState & ks, DrawManager &dm, const Press &p, float alpha) {
    ...
    std::pair<ofVec3f, ofVec3f> lineSegment = edgeToEdgeLineSegment(p, lineSegmentDeterministic ? p.id : (p.id + ks.randomSeed % SHAPE_PRIME));
    ofVec3f from = lineSegment.first;
    ofVec3f to = lineSegment.second;
    
    float computedDampenRadius = getGlowDampenRatio(glowIntensity,
                                                    intensityAtEighthWidth,
                                                    std::min(ofGetWidth(), ofGetHeight()) / 8.0);
    dm.shadeGlowLine(to + (from - to) * 2, from + (to - from) * 2, clr.color(p, alpha), glowIntensity, computedDampenRadius, blendMode, toneMap);
}
