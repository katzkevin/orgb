...
void BaseWaves::drawUnit(ColorProvider &clr, KeyState & ks, DrawManager &dm, const Press &p, float alpha) {
    ...
    float computedDampenRadius = getGlowDampenRatio(glowIntensity * ks.arousalGain(),
                                                    intensityAtEighthWidth,
                                                    std::min(ofGetWidth(), ofGetHeight()) / 8.0);
    dm.shadeGlowLine(ofVec2f(centerOfNotesZone, 0), ofVec2f(centerOfNotesZone, ofGetHeight()), clr.color(p, alpha), glowIntensity, computedDampenRadius, blendMode, toneMap);
}
...
void BaseWaves::draw(KeyState& ks, ColorProvider& clr, DrawManager& dm) {
    ...
    for (const auto &press : ks.allPresses()) {
        double a = press.audibleAmplitudePct(ks.attackTimeS, ks.decayTimeS, ks.sustainLevelPct, ks.releaseTimeS) * exponentialMap(press.velocityPct, 0, 1, 0, 1, true, 1 / 8.0);
