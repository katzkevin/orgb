...
void Orbit::update(KeyState & ks, ColorProvider & clr) {
    ...
    for (auto it = particles.begin(); it != particles.end(); ++it) {
        ...
        it->second.updatePosition(center +
                                  sphericalToRectangular(minSide, theta, phi));
    }
}
...
void Orbit::draw(KeyState& ks, ColorProvider& clr, DrawManager& dm) {
    ...
    for (int i = 0; i < NUM_NOTES; i++) {
        ...
        for (auto it = range.first; it != range.second; ++it) {
            float alpha = it->second.audibleAmplitudePct(ks.attackTimeS, ks.decayTimeS, ks.sustainLevelPct, ks.releaseTimeS);
            dm.shadeGlowCircle(ofVec2f(particle.position.x, particle.position.y), orbRadius, clr.color(it->second, alpha), glowIntensity, computedDampenRadius, blendMode, toneMap);
        }
    }
