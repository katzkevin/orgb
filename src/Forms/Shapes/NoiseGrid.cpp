...
void NoiseGrid::drawUnit(const ofColor &color, KeyState & ks, DrawManager &dm, Press &press) {
    ...
    float zVelocityPerSecond = exponentialMap(press.noteOverallPct(), 0, 1, baseZVelocityPerSecond, baseZVelocityPerSecond * topToBottomZVelocityRatio, true, 2.0);
...
//
    ...
    for (int i = 0; i < w; i += resolution) {
        for (int j = 0; j < h; j += resolution) {
            ...
            float noise = ofNoise(i / shortSide * frequency + pressOffset, j / shortSide * frequency, z + press.note * 100.0) * noiseAmplitude;
