...
void MeshGrid::drawUnit(const ofColor & color, KeyState & ks, DrawManager & dm, Press & press) {
    ...
    float period = exponentialMap(press.noteOverallPct(), 1, 0, minPeriodSeconds,
                                  minPeriodSeconds * topToBottomPeriodRatio, true, 2.0);
    ...
    while (offset <= longer) {
        ...
        //            float computedDampenRadius = getGlowDampenRatio(glowIntensity, intensityAtEighthWidth,
        //            std::min(ofGetWidth(), ofGetHeight()) / 8.0);
        //
        //            dm.shadeGlowLine(ofVec2f(-longer, offset), ofVec2f(longer, offset), color,
        //            functionalGlowIntensity, computedDampenRadius, blendMode, toneMap);
        //            dm.shadeGlowLine(ofVec2f(-longer, -offset), ofVec2f(longer, -offset), color,
        //            functionalGlowIntensity, computedDampenRadius, blendMode, toneMap);
