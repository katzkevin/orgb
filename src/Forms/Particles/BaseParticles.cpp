...
void BaseParticles::update(KeyState & ks, ColorProvider & clr) {
    ...
    for (auto press : ks.activePresses()) {
        if (ofIsFloatEqual(press.audibleAmplitudePct(ks.attackTimeS, ks.decayTimeS, ks.sustainLevelPct, ks.releaseTimeS), 0.0)) {
            generatingPressCount++;
        }
    }
    
    for (auto press : ks.activePresses()) {
        double audibleAmplitude = press.audibleAmplitudePct(ks.attackTimeS, ks.decayTimeS, ks.sustainLevelPct, ks.releaseTimeS);
        // Use squareRoot so we favor new presses over decaying ones. Particles should fall off more rapidly than, say, shapes.
        double squareRootOfAudibleAmplitude = exponentialMap(audibleAmplitude, 0, 1, 0, 1, true, 0.5);
        int numberOfParticlesToCreate = particleRate * ofGetLastFrameTime() * squareRootOfAudibleAmplitude * particleMultiplier + ofRandom(-0.5, 0.5);
        if (generatingPressCount > CONCURRENT_PRESS_PARTICLE_BRAKE) {
            numberOfParticlesToCreate = numberOfParticlesToCreate * (CONCURRENT_PRESS_PARTICLE_BRAKE / generatingPressCount) * particleMultiplier;
        }
        if (particles.size() > maxParticles) {
            numberOfParticlesToCreate = numberOfParticlesToCreate / (particles.size() / static_cast<float>(maxParticles));
        }
    ...
    for (auto press : ks.allEphemeralPresses()) {
        int numberOfParticlesToCreate =
            particleRate * ofGetLastFrameTime() * pow(press.velocityPct, 0.2) * particleMultiplier + ofRandom(-0.5, 0.5);
        if (particles.size() > maxParticles) {
            numberOfParticlesToCreate = numberOfParticlesToCreate / (particles.size() / static_cast<float>(maxParticles));
        }
    ...
    for (auto it = particles.begin(); it != particles.end(); ++it) {
        ofVec3f noiseGradientAtCoordinate = noiseGradientForCoordinates(it->second.position.x, it->second.position.y, noiseSpatialFrequency * (1 - ks.arousalGain()), noiseTemporalRate, noiseScale * (1 + ks.valenceGain()) / 2, timeS);
        ofVec3f acceleration = noiseScale > 0 ? ofVec3f(noiseGradientAtCoordinate.x, noiseGradientAtCoordinate.y,0) : ofVec3f(0,0,0);
...
void BaseParticles::pruneParticles() {
    ...
    for (auto iter = particles.begin(); iter != particles.end();) {
        if (iter->second.position.x > ofGetWidth() ||
            iter->second.position.x < 0 || iter->second.position.y > ofGetHeight() ||
            iter->second.position.y < 0 || iter->second.color.a < 0.0001) {
...
float BaseParticles::opacityForEphemeralPress(KeyState & ks, const Press & p) const {
    return 1.0;
}

float BaseParticles::opacityForPress(KeyState & ks, const Press & p) const {
    return 1.0;
}
