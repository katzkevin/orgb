...
void RadialParticles::createParticlesForPress(Press &press, int numberOfParticlesToCreate, ofColor c, float arousalPct) {
    ...
    for (int i = 0; i < numberOfParticlesToCreate; i++) {
        ...
        float velocity = ofMap(press.note, GUITAR_MIDI_MIN, GUITAR_MIDI_MAX, initialVelocityLowerBound, initialVelocityLowerBound * topToBottomInitialVelocityRatio, true) * ofRandom(0.8, 1.2) * arousalModifier;
        ofVec3f initialVelocity = randomUnitVector2D() * velocity;
        //ofVec3f scoot = (1.0 / TARGET_FRAME_RATE * ofRandom(0, 0.001)) * initialVelocity; // In order to avoid banding at the start point, scoot a bit into the framerate
