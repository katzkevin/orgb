...
void RandomParticles::createParticlesForPress(Press & press, int numberOfParticlesToCreate, ofColor c, float arousalPct) {
    ...
    for (int i = 0; i < numberOfParticlesToCreate; i++) {
        ...
        float velocity = ofMap(press.note, GUITAR_MIDI_MIN, GUITAR_MIDI_MAX, initialVelocityLowerBound, initialVelocityLowerBound * topToBottomInitialVelocityRatio, true) * arousalModifier;
...
float RandomParticles::opacityForEphemeralPress(KeyState & ks, const Press & p) const {
    return p.velocityPct;
}
