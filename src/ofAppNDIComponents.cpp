...
bool ofApp::ndiScanForVideoSources() {
    ...
    for (auto & source: sources) {
        if (ofStringTimesInString(source.p_ndi_name, resolutionSelector) > 0) {
            ofLogNotice("NDI") << "Found resolution selector match: " << resolutionSelector << " in " << source.p_ndi_name;
            return ndiSwitchToVideoSource(source);
        }
    }
...
bool ofApp::ndiUpdateHandler() {
    ...
    if(receiver_.isConnected()) {
        ...
        double scheduledAttemptDue = (lastNdiReconnectAttempt.is_initialized() ? lastNdiReconnectAttempt.is_initialized() : 0) + stoi(getEnv("NDI_SCAN_INTERVAL_SECONDS", "10"));
