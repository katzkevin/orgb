...
void main()
{
    ...
    float blurOffset = blurOffsetStepPixels / screenDimensions.y;
    vec4 color = vec4(0.0);

    color += 0.000229 * texture2D(tex0, texCoordVarying + blurOffset * vec2(0.0, -4.0));
    color += 0.005977 * texture2D(tex0, texCoordVarying + blurOffset * vec2(0.0, -3.0));
    color += 0.060598 * texture2D(tex0, texCoordVarying + blurOffset * vec2(0.0, -2.0));
    color += 0.241732 * texture2D(tex0, texCoordVarying + blurOffset * vec2(0.0, -1.0));
